...
<form method="GET" class="filter-row">
  ...
  <a href="{{ route('periti.index') }}" class="chip {{ !request('tipo') ? 'active' : '' }}">Tutti <span style="font-size:10px;opacity:.7">({{ $contatori['tutti'] }})</span></a>
  <a href="{{ route('periti.index') }}?tipo=perito" class="chip {{ request('tipo')==='perito' ? 'active' : '' }}">Periti <span style="font-size:10px;opacity:.7">({{ $contatori['perito'] }})</span></a>
  <a href="{{ route('periti.index') }}?tipo=avvocato" class="chip {{ request('tipo')==='avvocato' ? 'active' : '' }}">Avvocati <span style="font-size:10px;opacity:.7">({{ $contatori['avvocato'] }})</span></a>
  <a href="{{ route('periti.index') }}?tipo=legale" class="chip {{ request('tipo')==='legale' ? 'active' : '' }}">Legali <span style="font-size:10px;opacity:.7">({{ $contatori['legale'] }})</span></a>
        ...
        <td>
          <div style="display:flex;align-items:center;gap:9px">
            ...
            <div>
              ...
              @if($e->company_name)<div style="font-size:11px;color:var(--text3)">{{ $e->company_name }}</div>@endif
            </div>
          </div>
        </td>
        <td>
          @php $tc=['perito'=>'badge-purple','avvocato'=>'badge-orange','legale'=>'badge-orange','medico_legale'=>'badge-blue','liquidatore'=>'badge-teal','consulente'=>'badge-gray']; $tl=['perito'=>'Perito','avvocato'=>'Avvocato','legale'=>'Legale','medico_legale'=>'Medico Legale','liquidatore'=>'Liquidatore','consulente'=>'Consulente']; @endphp
