...
<form method="GET" class="filter-row">
  ...
  <a href="{{ route('sinistri.index') }}?status=liquidato" class="chip {{ request('status')==='liquidato' ? 'active' : '' }}">Liquidati <span style="font-size:10px;opacity:.7">({{ $contatori['liquidato'] }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=cid_presentato" class="chip {{ request('status')==='cid_presentato' ? 'active' : '' }}">CID <span style="font-size:10px;opacity:.7">({{ $contatori['cid_presentato'] ?? 0 }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=perizia_effettuata" class="chip {{ request('status')==='perizia_effettuata' ? 'active' : '' }}">Perizia effettuata <span style="font-size:10px;opacity:.7">({{ $contatori['perizia_effettuata'] ?? 0 }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=riparazione_completata" class="chip {{ request('status')==='riparazione_completata' ? 'active' : '' }}">Rip. completata <span style="font-size:10px;opacity:.7">({{ $contatori['riparazione_completata'] ?? 0 }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=liquidazione_attesa" class="chip {{ request('status')==='liquidazione_attesa' ? 'active' : '' }}">Liq. attesa <span style="font-size:10px;opacity:.7">({{ $contatori['liquidazione_attesa'] ?? 0 }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=contestato" class="chip {{ request('status')==='contestato' ? 'active' : '' }}">Contestati <span style="font-size:10px;opacity:.7">({{ $contatori['contestato'] ?? 0 }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=chiuso" class="chip {{ request('status')==='chiuso' ? 'active' : '' }}">Chiusi <span style="font-size:10px;opacity:.7">({{ $contatori['chiuso'] }})</span></a>
  <a href="{{ route('sinistri.index') }}?status=archiviato" class="chip {{ request('status')==='archiviato' ? 'active' : '' }}">Archiviati <span style="font-size:10px;opacity:.7">({{ $contatori['archiviato'] }})</span></a>
</form>
...
{{ $sinistri->appends(request()->query())->links('vendor.pagination.custom') }}
@endsection