...
<div class="two-col">
    ...
    <div class="card">
      ...
      <div class="form-group"><label class="form-label">Veicolo</label>
        <select name="vehicle_id" class="form-select">
          ...
          @foreach($veicoli as $v)<option value="{{ $v->id }}" {{ old('vehicle_id') == $v->id ? 'selected' : '' }}>{{ $v->plate }} — {{ $v->brand }} {{ $v->model }}</option>@endforeach
        </select></div>
      ...
      <div class="two-col" style="gap:10px">
        ...
        <div class="form-group"><label class="form-label">Importo stimato €</label><input type="number" name="estimated_amount" class="form-input" value="{{ old('estimated_amount') }}" step="0.01"></div>
      </div>
  ...
  <div>
    <div class="card">
      ...
      <div class="form-group"><label class="form-label">Targa controparte</label><input name="counterpart_plate" class="form-input" style="text-transform:uppercase" value="{{ old('counterpart_plate') }}"></div>
      <div class="form-group"><label class="form-label">Compagnia controparte</label><input name="counterpart_insurance" class="form-input" value="{{ old('counterpart_insurance') }}"></div>
    </div>
    <div class="card">
    ...
    <div class="card">
      ...
      <div class="form-group"><label class="form-label">N° Polizza</label><input name="policy_number" class="form-input" value="{{ old('policy_number') }}"></div>
      ...
      <div class="form-group"><label class="form-label">Liquidatore assegnato</label>
        <select name="liquidatore_id" class="form-select">
          <option value="">— Nessuno —</option>
          @foreach($liquidatori as $l)<option value="{{ $l->id }}" {{ old('liquidatore_id') == $l->id ? 'selected' : '' }}>{{ $l->name }}</option>@endforeach
        </select></div>
    </div>
