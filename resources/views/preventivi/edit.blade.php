@extends('layouts.app')
@section('title', 'Modifica Preventivo #'.$preventivo->quote_number)

@section('topbar-actions')
<a href="{{ route('preventivi.show', $preventivo) }}" class="btn btn-ghost btn-sm">← Torna al preventivo</a>
@endsection

@section('content')
<form method="POST" action="{{ route('preventivi.update', $preventivo) }}" id="form-preventivo">
@csrf @method('PUT')
...
<div style="display:grid;grid-template-columns:1fr 320px;gap:16px;align-items:start">
  ...
  <div>
    ...
    <div class="card">
      <div class="card-title">📋 Intestazione — <span style="font-family:var(--mono);color:var(--orange)">{{ $preventivo->quote_number }}</span></div>
      <div class="two-col">
          ...
          <select name="customer_id" class="form-select" required id="sel-cliente" onchange="onClienteChange(this.value)">
            ...
            @foreach($clienti as $c)
              <option value="{{ $c->id }}"
                ...
                {{ old('customer_id', $preventivo->customer_id) == $c->id ? 'selected' : '' }}>
                {{ $c->display_name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="form-group">
          ...
          <select name="job_type" class="form-select" required>
            @foreach([
              ...
              <option value="{{ $v }}" {{ old('job_type', $preventivo->job_type) === $v ? 'selected' : '' }}>{{ $l }}</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
          ...
          <select name="status" class="form-select">
            @foreach(['bozza'=>'Bozza','inviato'=>'Inviato','accettato'=>'Accettato','rifiutato'=>'Rifiutato','scaduto'=>'Scaduto'] as $v=>$l)
              <option value="{{ $v }}" {{ old('status', $preventivo->status) === $v ? 'selected' : '' }}>{{ $l }}</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
          ...
          <input type="date" name="valid_until" class="form-input"
            value="{{ old('valid_until', $preventivo->valid_until?->format('Y-m-d')) }}">
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Descrizione lavori</label>
        <textarea name="description" class="form-textarea" rows="3"
          placeholder="Descrivi i lavori da eseguire...">{{ old('description', $preventivo->description) }}</textarea>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div id="no-righe" style="text-align:center;padding:24px;color:var(--text3);font-size:13px;display:none">
    ...
    <div class="card">
      ...
      <textarea name="notes" class="form-textarea" rows="3"
        placeholder="Note per il cliente...">{{ old('notes', $preventivo->notes) }}</textarea>
    </div>
  ...
  <div>
    ...
    <div class="card">
      ...
      <select name="claim_id" class="form-select">
        ...
        @foreach($sinistri as $s)
          <option value="{{ $s->id }}" {{ old('claim_id', $preventivo->claim_id) == $s->id ? 'selected' : '' }}>
      ...
      <div class="two-col" style="gap:10px;margin-bottom:14px">
        <div class="form-group" style="margin-bottom:0">
          ...
          <input type="number" name="discount_percent" id="inp-sconto" class="form-input"
            value="{{ old('discount_percent', $preventivo->discount_percent) }}" min="0" max="100" step="0.01" oninput="ricalcola()">
        </div>
        <div class="form-group" style="margin-bottom:0">
          <label class="form-label">IVA %</label>
          <input type="number" name="vat_percent" id="inp-iva" class="form-input"
            value="{{ old('vat_percent', $preventivo->vat_percent) }}" min="0" max="100" step="0.01" oninput="ricalcola()">
        </div>
      </div>
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ...
      <a href="{{ route('preventivi.show', $preventivo) }}" class="btn btn-ghost" style="width:100%;justify-content:center">Annulla</a>
    </div>
...
let rigaIdx = 0;
const tipiVoce = {
  'manodopera':'🔧 Manodopera','ricambio':'🔩 Ricambio',
  'materiale':'🪣 Materiale','servizio':'⚙️ Servizio','altro':'📁 Altro'
};
...
function addRiga(data) {
  ...
  tr.innerHTML = `
    <td style="padding:6px 8px"><select name="items[${i}][item_type]" class="form-select" style="font-size:12px;padding:5px 8px">${tipoOpts}</select></td>
    <td style="padding:6px 8px"><input type="text" name="items[${i}][description]" class="form-input" style="font-size:12px;padding:5px 8px" placeholder="Descrizione..." value="${(d.description||'').replace(/"/g,'&quot;')}"></td>
    <td style="padding:6px 8px"><input type="number" name="items[${i}][quantity]" class="form-input riga-qta" style="font-size:12px;padding:5px 8px;text-align:right" value="${d.quantity||1}" min="0.01" step="0.01" oninput="aggiornaRiga(${i})"></td>
    <td style="padding:6px 8px"><input type="number" name="items[${i}][unit_price]" class="form-input riga-prezzo" style="font-size:12px;padding:5px 8px;text-align:right" value="${d.unit_price||''}" min="0" step="0.01" placeholder="0,00" oninput="aggiornaRiga(${i})"></td>
    <td style="padding:6px 8px"><input type="number" name="items[${i}][discount_percent]" class="form-input riga-sc" style="font-size:12px;padding:5px 8px;text-align:right" value="${d.discount_percent||0}" min="0" max="100" step="0.01" oninput="aggiornaRiga(${i})"></td>
    <td style="padding:6px 8px;text-align:right"><span id="tot-${i}" style="font-family:var(--mono);font-size:13px;font-weight:600;color:var(--green-text)">€ 0,00</span></td>
    <td style="padding:6px 4px;text-align:center"><button type="button" onclick="removeRiga(${i})" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:16px;padding:2px 6px">×</button></td>
  `;
...
function removeRiga(i) {
  ...
  if (tr) tr.remove();
  if (!document.getElementById('righe-body').children.length)
    document.getElementById('no-righe').style.display = '';
  ricalcola();
}
...
function aggiornaRiga(i) {
  ...
  document.getElementById(`tot-${i}`).textContent = '€ ' + tot.toFixed(2).replace('.', ',');
  ricalcola();
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (cid) {
    onClienteChange(cid);
    document.getElementById('sel-vehicle').value = '{{ $preventivo->vehicle_id }}';
  }
  const existingItems = @json($preventivo->items->sortBy('sort_order')->values());
  if (existingItems.length > 0) {
    existingItems.forEach(item => addRiga(item));
  } else {
    addRiga();
  }
});
