...
<div style="max-width:800px">
  <div style="margin-bottom:16px">
    <a href="{{ route('lavorazioni.index') }}" style="font-size:13px;color:var(--text3);text-decoration:none">← Torna alle Lavorazioni</a>
  </div>
  <div class="card">
        ...
        <div class="form-group">
          ...
          <select name="customer_id" class="form-select" required id="sel-cliente" onchange="loadVehicles(this.value)">
            ...
            @foreach($clienti as $c)
              <option value="{{ $c->id }}" {{ old('customer_id')==$c->id?'selected':'' }}>{{ $c->full_name }}</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <select name="claim_id" class="form-select">
            ...
            @foreach($sinistri as $s)
              <option value="{{ $s->id }}" {{ old('claim_id')===$s->id?'selected':'' }}>#{{ $s->claim_number }} — {{ $s->customer?->full_name }}</option>
            @endforeach
          </select>
        </div>
      </div>
