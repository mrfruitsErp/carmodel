...
:root{
  ...
  --radius:6px;--radius-lg:10px;--sidebar:175px;
}
...
.sidebar::-webkit-scrollbar{width:6px}.sidebar::-webkit-scrollbar-track{background:#0d1117}.sidebar::-webkit-scrollbar-thumb{background:#ff6b00;border-radius:4px}.sidebar::-webkit-scrollbar-thumb:hover{background:#ff8c00}
.main{margin-left:var(--sidebar);flex:1;display:flex;flex-direction:column;min-height:100vh;min-width:0}
.logo{padding:12px 12px 10px;border-bottom:1px solid rgba(255,255,255,.06);position:relative}
...
.nav-section{color:rgba(255,255,255,.25);font-size:9px;font-weight:600;padding:10px 12px 3px;letter-spacing:.15em;text-transform:uppercase}
.nav-item{display:flex;align-items:center;gap:7px;padding:6px 12px;font-size:11px;color:rgba(255,255,255,.55);transition:all .15s;border-left:2px solid transparent;text-decoration:none;margin:0 3px;border-radius:5px;white-space:nowrap;overflow:hidden}
.nav-item:hover{background:rgba(255,255,255,.06);color:rgba(255,255,255,.9)}
.nav-item.active{background:rgba(255,107,0,.12);color:var(--orange);border-left:2px solid var(--orange);font-weight:600;margin-left:3px;border-radius:0 5px 5px 0}
...
<div class="sidebar">
  <div class="logo">
    <img src="{{ asset('images/logo_alecar.jpg') }}" alt="AleCar" style="height:40px;width:auto;object-fit:contain;display:block">
  </div>
  <nav class="nav">
    <div class="nav-section">Principale</div>
    <a href="{{ route('dashboard') }}" class="nav-item {{ request()->routeIs('dashboard') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>Dashboard
    </a>
    <div class="nav-section">Anagrafica</div>
    <a href="{{ route('clienti.index') }}" class="nav-item {{ request()->routeIs('clienti.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>Clienti
    </a>
    <a href="{{ route('veicoli.index') }}" class="nav-item {{ request()->routeIs('veicoli.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 17H3v-5l2-5h14l2 5v5h-2"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="16.5" cy="17.5" r="2.5"/></svg>Veicoli
    </a>
    <div class="nav-section">Sinistri & Lesioni</div>
    <a href="{{ route('sinistri.index') }}" class="nav-item {{ request()->routeIs('sinistri.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/></svg>Sinistri
    </a>
    <a href="{{ route('lesioni.index') }}" class="nav-item {{ request()->routeIs('lesioni.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Lesioni Personali
    </a>
    <a href="{{ route('periti.index') }}" class="nav-item {{ request()->routeIs('periti.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Periti & Avvocati
    </a>
    <div class="nav-section">Officina</div>
    <a href="{{ route('lavorazioni.index') }}" class="nav-item {{ request()->routeIs('lavorazioni.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>Lavorazioni
    </a>
    <a href="{{ route('preventivi.index') }}" class="nav-item {{ request()->routeIs('preventivi.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/></svg>Preventivi
    </a>
    <div class="nav-section">Noleggio</div>
    <a href="{{ route('flotta.index') }}" class="nav-item {{ request()->routeIs('flotta.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg>Flotta Veicoli
    </a>
    <a href="{{ route('noleggio.index') }}" class="nav-item {{ request()->routeIs('noleggio.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>Contratti Noleggio
    </a>
    <a href="{{ route('sostitutive.index') }}" class="nav-item {{ request()->routeIs('sostitutive.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>Auto Sostitutive
    </a>
    <div class="nav-section">Amministrazione</div>
    <a href="{{ route('documenti.index') }}" class="nav-item {{ request()->routeIs('documenti.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>Fatture & DDT
    </a>
    <a href="{{ route('mail.index') }}" class="nav-item {{ request()->routeIs('mail.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>Mail & Notifiche
    </a>
    <a href="{{ route('ricambi.index') }}" class="nav-item {{ request()->routeIs('ricambi.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>Ricambi
    </a>
    ...
    <a href="{{ route('marketplace.vehicles.index') }}" class="nav-item {{ request()->routeIs('marketplace.vehicles.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 17H3v-5l2-5h14l2 5v5h-2"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="16.5" cy="17.5" r="2.5"/></svg>Veicoli in vendita
    </a>
    <a href="{{ route('marketplace.leads.index') }}" class="nav-item {{ request()->routeIs('marketplace.leads.*') ? 'active' : '' }}">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>Lead
    </a>
  ...
  <div class="user-area">
    <div style="display:flex;align-items:center;gap:8px">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:11px;font-weight:500;color:rgba(255,255,255,.8);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ auth()->user()->name }}</div>
