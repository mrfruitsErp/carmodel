@extends('layouts.app')
@section('title', $type === 'noleggio' ? 'Veicoli a noleggio' : 'Veicoli in vendita')

@section('topbar-actions')
@if($type === 'noleggio')
  <a href="{{ route('flotta.create') }}" class="btn btn-primary btn-sm">+ Aggiungi veicolo a noleggio</a>
@else
...
{{-- TAB VENDITA / NOLEGGIO --}}
<div style="display:flex;gap:8px;margin-bottom:18px;border-bottom:1px solid var(--border2);padding-bottom:0">
  <a href="{{ route('marketplace.vehicles.index', ['type'=>'vendita']) }}"
     style="padding:10px 18px;font-size:13px;font-weight:600;text-decoration:none;border-bottom:3px solid {{ $type==='vendita' ? 'var(--orange)' : 'transparent' }};color:{{ $type==='vendita' ? 'var(--orange)' : 'var(--text3)' }};margin-bottom:-1px;display:flex;align-items:center;gap:8px">
     🚗 Vendita
  </a>
  <a href="{{ route('marketplace.vehicles.index', ['type'=>'noleggio']) }}"
     style="padding:10px 18px;font-size:13px;font-weight:600;text-decoration:none;border-bottom:3px solid {{ $type==='noleggio' ? 'var(--orange)' : 'transparent' }};color:{{ $type==='noleggio' ? 'var(--orange)' : 'var(--text3)' }};margin-bottom:-1px;display:flex;align-items:center;gap:8px">
     🔑 Noleggio
  </a>
</div>
...
@if($type === 'noleggio')
  ...
  <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:20px">
    @foreach([
      ['Totale flotta',  $rentalStats['total'],        'var(--blue)',   ''],
      ['Disponibili',    $rentalStats['disponibili'],  'var(--green)',  'disponibile'],
      ['In noleggio',    $rentalStats['in_noleggio'],  'var(--orange)', 'in_noleggio'],
      ['Manutenzione',   $rentalStats['manutenzione'], 'var(--amber)',  'manutenzione'],
      ['Web visibili',   $rentalStats['web_visibili'], 'var(--purple)', 'web'],
    ] as [$label, $val, $color, $filter])
    <a href="{{ route('marketplace.vehicles.index', array_filter(['type'=>'noleggio','status'=>$filter])) }}" style="text-decoration:none">
  ...
  <div style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap">
    @foreach([
      ...
      'in_noleggio'  => 'In noleggio',
      'manutenzione' => 'Manutenzione',
      'fuori_uso'    => 'Fuori uso',
    ] as $val=>$label)
    <a href="{{ route('marketplace.vehicles.index', array_merge(['type'=>'noleggio'], request()->except('status','page','type'), $val ? ['status'=>$val] : [])) }}"
  ...
  <form method="GET" style="display:flex;gap:10px;margin-bottom:20px;align-items:center;flex-wrap:wrap">
    <input type="hidden" name="type" value="noleggio">
    ...
    @if(request()->hasAny(['search','status']))
      <a href="{{ route('marketplace.vehicles.index', ['type'=>'noleggio']) }}" class="btn btn-ghost btn-sm">✕ Reset</a>
    @endif
  ...
  @if($vehicles->isEmpty())
    <div style="background:var(--bg2);border:1px solid var(--border2);border-radius:12px;padding:60px 24px;text-align:center">
      ...
      <div style="font-size:16px;font-weight:600;color:var(--text);margin-bottom:8px">Nessun veicolo a noleggio</div>
      <div style="font-size:13px;color:var(--text3);margin-bottom:20px">La flotta è vuota o nessun veicolo corrisponde ai filtri</div>
      <a href="{{ route('flotta.create') }}" class="btn btn-primary">+ Aggiungi veicolo a noleggio</a>
    </div>
  @else
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px">
    ...
    @php
      $statusCfg = match($vehicle->status) {
        'disponibile'  => ['bg'=>'var(--green-bg)',  'color'=>'var(--green-text)',  'label'=>'Disponibile',  'dot'=>'var(--green)'],
        'in_noleggio'  => ['bg'=>'var(--orange-bg)' ?? 'var(--amber-bg)','color'=>'var(--orange-text)' ?? 'var(--amber-text)','label'=>'In noleggio','dot'=>'var(--orange)'],
        'manutenzione' => ['bg'=>'var(--amber-bg)',  'color'=>'var(--amber-text)',  'label'=>'Manutenzione', 'dot'=>'var(--amber)'],
        'fuori_uso'    => ['bg'=>'var(--red-bg)',    'color'=>'var(--red-text)',    'label'=>'Fuori uso',    'dot'=>'var(--red)'],
        default        => ['bg'=>'var(--bg4)',       'color'=>'var(--text3)',       'label'=>ucfirst($vehicle->status ?? '—'),'dot'=>'var(--border3)'],
      };
      $rate = $vehicle->daily_rate_public ?: $vehicle->daily_rate;
      $activeRentals = $vehicle->rentals()->whereNotIn('status', ['chiuso','annullato'])->count();
    @endphp
    <div style="background:var(--bg2);border:1px solid var(--border2);border-radius:12px;overflow:hidden;transition:all .2s" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.1)'" onmouseout="this.style.transform='';this.style.boxShadow=''">
      ...
      <a href="{{ route('flotta.show', $vehicle) }}" style="display:block;position:relative;height:180px;background:#f0f0f0;overflow:hidden">
        <div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#fef3e7,#fde4c0)">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#d97706" stroke-width="1.5"><path d="M5 17H3v-5l2-5h14l2 5v5h-2"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="16.5" cy="17.5" r="2.5"/><path d="M12 8v4M12 8l-2 2M12 8l2 2"/></svg>
          <span style="font-size:11px;color:#a16207;margin-top:8px;font-weight:600">NOLEGGIO</span>
        </div>
        ...
        @if($vehicle->web_visible)
          <div style="position:absolute;top:10px;right:10px;background:rgba(139,92,246,.9);color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:10px">🌐 Web</div>
        @endif
        @if($vehicle->booking_enabled)
          <div style="position:absolute;bottom:10px;right:10px;background:rgba(16,185,129,.9);color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:10px">📅 Prenotabile</div>
        @endif
      </a>
      ...
      <div style="padding:14px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
          <div style="min-width:0">
            ...
            <div style="font-size:11px;color:var(--text3);margin-top:1px;font-family:var(--mono)">{{ $vehicle->plate }}</div>
          </div>
          <div style="text-align:right;flex-shrink:0;margin-left:8px">
            @if($rate)
              <div style="font-size:16px;font-weight:800;color:var(--orange)">{{ number_format($rate,0,',','.') }} &euro;</div>
              <div style="font-size:10px;color:var(--text3)">al giorno</div>
            @else
              <div style="font-size:11px;color:var(--text3)">tariffa n.d.</div>
            @endif
          </div>
        </div>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">
          ...
          @endif
          @if($vehicle->category)
            <span style="font-size:11px;color:var(--text3);background:var(--bg3);padding:2px 7px;border-radius:4px">{{ ucfirst($vehicle->category) }}</span>
          @endif
          @if($vehicle->seats)
            <span style="font-size:11px;color:var(--text3);background:var(--bg3);padding:2px 7px;border-radius:4px">{{ $vehicle->seats }} posti</span>
          @endif
          @if($vehicle->km_current)
            <span style="font-size:11px;color:var(--text3);background:var(--bg3);padding:2px 7px;border-radius:4px">{{ number_format($vehicle->km_current,0,',','.') }} km</span>
          @endif
        </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding-top:10px;border-top:1px solid var(--border)">
          <div style="display:flex;gap:12px;align-items:center;font-size:11px;color:var(--text3)">
            @if($activeRentals > 0)
              <span style="color:var(--green-text);font-weight:600">{{ $activeRentals }} noleggio attivo</span>
            @endif
            @if($vehicle->insurance_expiry && $vehicle->insurance_expiry->diffInDays(now()) <= 30)
              <span style="color:var(--amber)">⚠ Assic. in scadenza</span>
            @endif
          </div>
          <a href="{{ route('flotta.show', $vehicle) }}" style="font-size:12px;font-weight:600;color:var(--orange);text-decoration:none">
            Gestisci &rsaquo;
          </a>
        </div>
      </div>
...
@else
  ...
  <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:20px">
    ...
    <a href="{{ route('marketplace.vehicles.index', array_filter(['type'=>'vendita','status'=>$filter])) }}" style="text-decoration:none">
      <div style="background:var(--bg2);border:1px solid {{ request('status')===$filter && $filter ? $color : 'var(--border2)' }};border-radius:10px;padding:14px 16px;position:relative;overflow:hidden;transition:.15s">
        <div style="position:absolute;top:0;left:0;right:0;height:2px;background:{{ $color }}"></div>
        <div style="font-size:10px;color:var(--text3);font-weight:600;letter-spacing:.1em;text-transform:uppercase;margin-bottom:8px">{{ $label }}</div>
        <div style="font-family:var(--font-display);font-size:26px;font-weight:700;color:var(--text);line-height:1">{{ $val }}</div>
  ...
  <div style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap">
    ...
    <a href="{{ route('marketplace.vehicles.index', array_merge(['type'=>'vendita'], request()->except('status','page','type'), $val ? ['status'=>$val] : [])) }}"
      style="padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;text-decoration:none;transition:.15s;
        border:1px solid {{ request('status')===$val ? 'var(--orange)' : 'var(--border2)' }};
        background:{{ request('status')===$val ? 'var(--orange)' : 'var(--bg2)' }};
        color:{{ request('status')===$val ? '#000' : 'var(--text2)' }}">
  ...
  {{-- SEARCH --}}
  <form method="GET" style="display:flex;gap:10px;margin-bottom:20px;align-items:center;flex-wrap:wrap">
    <input type="hidden" name="type" value="vendita">
    @if(request('status'))<input type="hidden" name="status" value="{{ request('status') }}">@endif
    <div class="search-bar" style="max-width:280px">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--text3)"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
      <input type="text" name="search" value="{{ request('search') }}" placeholder="Marca, modello, targa...">
    ...
    <button type="submit" class="btn btn-ghost btn-sm">Cerca</button>
    @if(request()->hasAny(['search','status']))
      <a href="{{ route('marketplace.vehicles.index', ['type'=>'vendita']) }}" class="btn btn-ghost btn-sm">✕ Reset</a>
    @endif
    <div style="margin-left:auto;font-size:12px;color:var(--text3)">{{ $vehicles->total() }} veicoli</div>
  </form>
