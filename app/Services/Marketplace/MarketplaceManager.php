...
class MarketplaceManager
{
    ...
    public function dashboardStats(int $tenantId): array
    {
        return [
            ...
            'vehicles_archived'  => SaleVehicle::forTenant($tenantId)->where('status', 'archiviato')->count(),
            'listings_published' => MarketplaceListing::forTenant($tenantId)->where('status', 'published')->count(),
            'listings_live'      => MarketplaceListing::forTenant($tenantId)->where('status', 'published')->count(),
